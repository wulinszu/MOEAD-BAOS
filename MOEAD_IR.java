...
public class MOEAD_IR extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		do {
			...
			for (int i = 0; i < order.size(); i++) {
				...
				parents = matingSelection(p, n, 5, type);
  ...
  public void initPopulation() throws JMException, ClassNotFoundException {
	...
	public Solution[] matingSelection(Vector<Integer> list, int cid, int size, int type) {
		...
		Solution[] parents = new Solution[size];
		...
		parents[2] = population_.get(list.get(2));
		parents[3] = population_.get(list.get(3));
		parents[4] = population_.get(list.get(4));
